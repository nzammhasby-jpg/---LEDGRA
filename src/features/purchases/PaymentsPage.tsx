...
export const PaymentsPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const [allPayments, allVendors, allBills, allAccounts, taxSettings, allCashBankAccounts] = await Promise.all([
        purchaseService.getPayments(currentOrg!.id, { showDeleted }),
  ...
  const handleConfirmSoftDelete = async () => {
    ...
    setError(null);
    try {
      await purchaseService.softDeletePayment(deletingId, deleteReason);
      setViewState('list');
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 font-sans text-right" dir="rtl">
      ...
      {deleteConfirmOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4" id="soft-delete-modal-payments">
