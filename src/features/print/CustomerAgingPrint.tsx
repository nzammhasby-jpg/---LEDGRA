...
export const CustomerAgingPrint: React.FC = () => {
  ...
  const loadReport = async () => {
    ...
    setError(null);
    try {
      const data = await reportsService.getCustomerAgingReport(currentOrg!.id, asOfDate);
      setReportData(data || []);
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6" dir="rtl">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 font-bold">جاري موازنة وتحليل تقرير أعمار الديون للعملاء...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6" dir="rtl">
        <div className="max-w-md w-full bg-white border border-red-200 rounded-3xl p-6 text-center space-y-4 shadow-xl">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800 font-sans">تعذر بناء تقرير أعمار ذمم العملاء</h3>
    ...
    <div className="bg-slate-100 min-h-screen">
      ...
      <div className="relative bg-white w-full max-w-[210mm] min-h-[297mm] mx-auto p-12 my-8 border border-slate-200 shadow-2xl rounded-xl print-page print:border-none print:shadow-none print:my-0 print:p-0 print:rounded-none overflow-hidden">
        ...
        <PrintHeader
          ...
          documentTitle="تقرير أعمار ذمم العملاء (ميزان أعمار الديون)"
          documentNumber=""
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4.5 mb-8 text-right font-sans select-none" dir="rtl">
          <div className="grid grid-cols-4 gap-2.5 text-center text-xs">
            <div className="bg-white border rounded-xl p-2.5">
              <span className="text-[9px] text-slate-400 block mb-1">إجمالي الذمم المستحقة:</span>
            ...
            <div className="bg-white border border-red-200 rounded-xl p-2.5 text-red-650 bg-red-50/10">
              <span className="text-[9px] text-red-400 block mb-1">الديون المتقادمة (&gt; 90 يوم):</span>
          ...
          <table className="w-full border-collapse text-xs select-none">
            <thead>
              <tr className="bg-slate-900 text-white rounded-lg">
                <th className="py-2.5 px-3 border border-slate-900 font-extrabold text-right">العميل</th>
              ...
              {reportData.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 px-3 border border-slate-200 text-center text-slate-400">
                    لا توجد أرصدة مستحقة لعملاء المنشأة حتى تاريخ التقرير.
                  </td>
                </tr>
              ) : (
                reportData.map((row) => (
                  <tr key={row.customer_id} className="hover:bg-slate-50 border-b border-slate-200">
                    <td className="py-2.5 px-3 text-right font-sans">
                      <div className="font-black text-slate-800">{row.customer_name}</div>
                      {row.customer_code && <div className="text-[10px] text-slate-400 mt-0.5">رمز العميل: {row.customer_code}</div>}
                    </td>
        ...
        <div className="bg-blue-50/10 border border-blue-200 rounded-xl p-3 mb-6 select-none leading-relaxed text-[11px]" dir="rtl">
          ...
          <p className="text-slate-600">
            يعتمد هذا التقرير على تواريخ استحقاق فواتير المبيعات المعتمدة ومطابقة الدفعات المخصصة لها حتى تاريخ الاستحقاق. لا يشمل الفواتير المسودة أو الحركات الملغاة/المحذوفة.
          </p>
        </div>

        <PrintFooter showSignatures={true} description="تقرير أعمار الذمم المدينة للعملاء صادر آلياً من نظام لِدجرا السحابي المعتمد." />
