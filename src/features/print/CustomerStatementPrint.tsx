...
import { reportsService, CustomerStatementResult } from '../../lib/reportsService';
...
export const CustomerStatementPrint: React.FC = () => {
  ...
  useEffect(() => {
    if (currentOrg?.id) {
      if (!customerId || !dateFrom || !dateTo) {
        setLoading(false);
        setError('يرجى اختيار العميل وتحديد الفترة الزمنية كاملة لعرض كشف الحساب المعتمد.');
        return;
      }
      loadStatement();
    }
  }, [currentOrg?.id, customerId, dateFrom, dateTo]);

  const loadStatement = async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await reportsService.getCustomerStatement(
        currentOrg!.id,
        customerId,
        dateFrom,
        dateTo
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6" dir="rtl">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 font-bold">جاري موازنة وتوليد كشف حساب العميل...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6" dir="rtl">
        <div className="max-w-md w-full bg-white border border-red-200 rounded-3xl p-6 text-center space-y-4 shadow-xl">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800 font-sans">تعذر بناء كشف الحساب</h3>
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">{error || 'العميل غير موجود أو المعطيات غير مكتملة'}</p>
          </div>
          <button
    ...
    <div className="bg-slate-100 min-h-screen">
      ...
      <div className="relative bg-white w-full max-w-[210mm] min-h-[297mm] mx-auto p-12 my-8 border border-slate-200 shadow-2xl rounded-xl print-page print:border-none print:shadow-none print:my-0 print:p-0 print:rounded-none overflow-visible print:overflow-visible">
        ...
        <PrintHeader
          ...
          documentTitle="كشف حساب عميل (تفصيلي)"
          documentNumber={statement.customer_code}
        ...
        {/* Client general info & balances index cards */}
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4.5 mb-8 text-right font-sans select-none" dir="rtl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-3 mb-3">
            <div>
              <span className="text-[10px] font-black text-slate-400 block uppercase mb-0.5">اسم العميل:</span>
              <h2 className="text-sm font-black text-slate-800">{statement.customer_name}</h2>
            </div>
            <div className="text-xs text-slate-500 font-mono">
              رمز العميل: <span className="font-bold text-slate-800">{statement.customer_code}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2.5 text-center text-xs">
            ...
            <div className="bg-white border border-emerald-100 rounded-xl p-2.5">
              <span className="text-[9px] text-slate-400 block mb-1">إجمالي الحركات المدينة (+):</span>
            ...
            <div className="bg-white border border-amber-105 rounded-xl p-2.5">
              <span className="text-[9px] text-slate-400 block mb-1">إجمالي الحركات الدائنة (-):</span>
            ...
            <div className={`border rounded-xl p-2.5 ${statement.closing_balance >= 0 ? 'bg-blue-50/10 border-blue-200 text-brand-blue' : 'bg-red-50/10 border-red-200 text-red-650'}`}>
              <span className="text-[9px] text-slate-400 block mb-1">الرصيد النهائي (الختامي):</span>
              ...
              <span className="text-[8px] block font-semibold mt-0.5">
                ({statement.closing_balance >= 0 ? 'مستحق على العميل للمنشأة' : 'رصيد دائن لصالح العميل'})
              </span>
            </div>
          </div>
        </div>

        {/* Dynamic movements records table */}
          ...
          <table className="w-full border-collapse text-xs select-none">
            <thead>
              <tr className="bg-slate-900 text-white rounded-lg">
                ...
                <th className="py-2 px-2.5 border border-slate-900 font-extrabold w-28 text-center">المرجع / القيد</th>
                <th className="py-2 px-2.5 border border-slate-900 font-extrabold text-right">البيان والشرح</th>
                <th className="py-2 px-2.5 border border-slate-900 font-extrabold w-24 text-center">مدين (+)</th>
                <th className="py-2 px-2.5 border border-slate-900 font-extrabold w-24 text-center">دائن (-)</th>
                <th className="py-2 px-2.5 border border-slate-900 font-extrabold w-28 text-left">الرصيد الجاري</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              ...
              <tr className="bg-slate-50 italic text-slate-500 print:break-inside-avoid">
                ...
                <td className="py-2 px-2.5 font-sans">** رصيد منقول من فترة سابقة **</td>
        ...
        <div className="bg-blue-50/10 border border-blue-200 rounded-xl p-3 mb-6 select-none leading-relaxed text-[11px]" dir="rtl">
          <span className="font-black text-slate-850 block mb-0.5">توضيح الأرصدة:</span>
          <p className="text-slate-600">
            تاريخ استحقاق الأرصدة يتبع تواريخ الفاتورة المعنية. يرجى مراجعة إدارة المعاملات التجارية لإجراء أي تسويات أو تفنيد للحركات الدائنة/المدينة المدرجة أعلاه خلال 7 أيام من تاريخ الاستخراج.
          </p>
        </div>

        {/* Dynamic standard footer signature board */}
        <PrintFooter showSignatures={true} description="كشف حساب تفصيلي يوضح كافة الحركات المدينة والدائنة المتبادلة والرصيد التراكمي خلال الفترة المحددة لأغراض المراجعة والمطابقة." />
