...
export const InventoryReportPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      {/* Top action row */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4.5 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <button
          onClick={fetchReport}
          disabled={loading}
          className="bg-brand-blue hover:bg-brand-blue/90 text-white text-xs font-bold px-5 py-2.25 rounded-xl transition flex items-center gap-2 cursor-pointer disabled:opacity-50 shrink-0 self-start sm:self-auto"
        >
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="border-b border-slate-100 bg-slate-50/50 p-4 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-405">تضمين التخصيصات والربط الحسابي للقيد المزدوج</span>
        </div>
              ...
              {filteredRows.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-8 text-xs text-slate-400 italic">
                    لا توجد منتجات مخزنية مضافة أو حركات مخزون رصيدية حتى الآن.
                  </td>
                </tr>
              ) : (
