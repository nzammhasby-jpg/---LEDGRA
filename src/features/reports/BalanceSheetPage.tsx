...
export const BalanceSheetPage: React.FC = () => {
  ...
  const handleExportCSV = () => {
    ...
    const csvRows: any[][] = [
      ...
      ['التقرير', 'الميزانية العمومية / المركز المالي التفصيلي'],
      ['حتى تاريخ الموقف', asOfDate],
      ['العملة المعتمدة', currency],
      ['الفرق الدفتري الفتري', reportData.check_difference],
      [],
      ['1. الأصول والممتلكات (Assets)'],
      ['كود الحساب', 'اسم الحساب', 'التصنيف الرئيسي', 'القيمة الدفترية'],
      ...assetsList.map(a => [a.account_code, a.account_name_ar, 'الأصول', a.amount]),
      ['المجموع الإجمالي للأصول', '', '', reportData.assets],
      [],
      ['2. الالتزامات والخصوم (Liabilities)'],
      ['كود الحساب', 'اسم الحساب', 'التصنيف الرئيسي', 'القيمة الدفترية'],
      ...liabilitiesList.map(l => [l.account_code, l.account_name_ar, 'الالتزامات', l.amount]),
      ['المجموع الإجمالي للالتزامات', '', '', reportData.liabilities],
      [],
      ['3. حقوق الملكية ورأس المال (Equity)'],
      ['كود الحساب', 'اسم الحساب', 'التصنيف الرئيسي', 'القيمة الدفترية'],
      ...equityList.map(e => [e.account_code, e.account_name_ar, 'حقوق الملكية', e.amount]),
      ['صافي أرباح العام المترتبة YTD', 'صافي أرباح العام الحالي (YTD)', 'حقوق الملكية', reportData.current_year_net_income],
      ['المجموع الإجمالي لحقوق الملكية ورأس المال', '', '', reportData.equity + reportData.current_year_net_income],
      [],
      ['إجمالي الجانب الأيسر (الخصوم + حقوق الملكية)', '', '', reportData.liabilities + reportData.equity + reportData.current_year_net_income]
    ];

    const headers = ['بند المركز المالي التفصيلي', 'تفاصيل الحسابات وقيم الأرصدة'];
    const csvContent = generateCSV(headers, csvRows);
    const filename = generateReportFilename('المركز_المالي', asOfDate);
    downloadCSV(csvContent, filename);
  };

  const handlePrint = () => {
    window.open(`#/print/balance-sheet?asOfDate=${asOfDate}`, '_blank');
  };

  return (
    <div className="space-y-6 text-right font-sans" dir="rtl">
      
      {/* Report Title Header Intro */}
      <div className="space-y-1">
        <h3 className="text-lg font-black text-slate-800">بيان الميزانية العمومية والمركز المالي</h3>
        <p className="text-xs text-slate-500">
          استعرض أرصدة أصول المنشأة والتزاماتها وحقوق الملكية الجارية حتى أي تاريخ موقف تريده للتحقق من توازن الميزانية ودقة أرصدتك الدفترية.
        </p>
      </div>
      ...
      {/* Action buttons row for Balance Sheet */}
      {reportData && !loading && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 print:hidden">
          <span className="text-xs font-bold text-slate-500">خيارات تصدير وطباعة المركز المالي:</span>
          <ReportActions
            onPrint={handlePrint}
      ...
      {loading && (
        <div className="bg-white p-12 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center gap-3 text-slate-500">
          ...
          <span className="text-xs font-bold">جاري تجميع أرصدة الميزانية العمومية والتحقق من التوازن الحركي...</span>
        </div>
      )}
      ...
      {/* Report Data view with Standard Header */}
      {reportData && !loading && (
        <div className="space-y-6">
          
          <ReportHeader
            reportName="تقرير الميزانية العمومية والمركز المالي"
            dateFrom=""
            dateTo={asOfDate}
            showSingleDateOnly={true}
          />
