...
import { reportsService, CustomerStatementResult, CustomerMovement } from '../../lib/reportsService';
...
export const CustomerStatementPage: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      setLoadingList(true);
      const custList = await masterDataService.getCustomers(currentOrg!.id);
      ...
      if (custList && custList.length > 0) {
        ...
        fetchReport(custList[0].id, `${cy}-01-01`, new Date().toISOString().split('T')[0]);
      }
    } catch (err) {
    ...
    <div className="space-y-6 text-right font-sans" dir="rtl">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-5 rounded-2xl border border-slate-100 flex flex-wrap gap-4 items-end shadow-sm">
        ...
        <div className="space-y-1.5 shrink-0 w-full sm:w-64">
          <label className="text-xs font-bold text-slate-500 block">اختر العميل</label>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-1">
              <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">الرصيد الافتتاحي (قبل الفترة)</span>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-1">
              <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">مجموع مبيعات ومستحقات (مدين)</span>
              <div className="flex items-center justify-between">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-1">
              <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">مجموع تحصيلات وسندات (دائن)</span>
              <div className="flex items-center justify-between">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-1">
              <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">صافي الرصيد المستحق النهائي</span>
              <div className="flex items-center justify-between">
                <span className={`text-lg font-extrabold font-sans ${reportData.closing_balance >= 0 ? 'text-brand-blue' : 'text-red-605'}`} style={{ direction: 'ltr' }}>
                  {formatNumberWithLatinDigits(reportData.closing_balance)}
                </span>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${reportData.closing_balance >= 0 ? 'bg-brand-blue/10 text-brand-blue' : 'bg-red-500/10 text-red-650'}`}>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="border-b border-slate-100 bg-slate-50/50 p-4 leading-normal flex items-center justify-between">
              ...
              <table className="w-full text-right text-xs">
                ...
                <thead>
                  <tr className="bg-slate-100/50 border-b border-slate-150 text-slate-500 select-none font-bold">
                    ...
                    <th className="py-3 px-4 text-left w-36 bg-slate-100/40">الرصيد التراكمي</th>
                  </tr>
                </thead>
                  ...
                  {reportData.movements.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="text-center py-8 text-xs text-slate-400 italic">
                        لا توجد حركات تجارية أو قيود أستاذ مرحلة لهذا العميل خلال الفترة المحددة.
                      </td>
                    </tr>
                  ) : (
            ...
            <div className="p-4 bg-slate-50/50 border-t border-slate-100 text-[10px] text-slate-450 flex items-center gap-1.5 flex-wrap">
              ...
              <span>ملاحظة: السندات المسجلة غير المرحلة لا تدخل في حساب كشف الحساب لضمان توافق الدفاتر والميزانية السنوية.</span>
            </div>
