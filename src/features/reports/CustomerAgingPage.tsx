...
export const CustomerAgingPage: React.FC = () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['أعمار ذمم العملاء التفصيلي', 'التفاصيل'];
    const csvContent = generateCSV(headers, csvRows);
    const filename = generateReportFilename(`اعمار_ذمم_العملاء`, asOfDate);
    downloadCSV(csvContent, filename);
  };

  const handlePrint = () => {
    if (!asOfDate) return;
    window.open(`#/print/customer-aging?asOfDate=${asOfDate}`, '_blank');
  };
  ...
  return (
    <div className="space-y-6 text-right font-sans" dir="rtl">
      ...
      <div className="space-y-1">
        <h3 className="text-lg font-black text-slate-800">تقرير أعمار ذمم العملاء</h3>
        <p className="text-xs text-slate-500">
          راقب حلول مستحقات وديون العملاء موزعة حسب الفترات الزمنية لتأمين السيولة وإدارة التحصيلات بكفاءة وتدقيق الحسابات المتأخرة.
        </p>
      </div>
