-- ==========================================================
-- LEDGRA ADVANCED FINANCIAL REPORTS PHASE C
-- ==========================================================

BEGIN;

-- 1. Create get_trial_balance_advanced
...
BEGIN
    ...
    -- Calculate Totals based on direct/non-rolled-up balances of all accounts to prevent double counting
    ...
    -- Build recursive tree rollup for display (which combines parent totals beautifully if p_include_parent_accounts = true)
    ...
    WHERE 
        -- include_parent_accounts filter (if false, exclude parent accounts)
        ...
        AND
        -- include_zero_accounts filter (if false, exclude accounts where all debit/credit columns are zero)
...
-- Secure grants for get_trial_balance_advanced
...
-- 2. Create get_ledger_report_advanced
...
BEGIN
    ...
    calculated_running AS (
        ...
        FROM period_lines
        ORDER BY entry_date ASC, created_at ASC, line_id ASC
    )
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'entry_id', entry_id,
